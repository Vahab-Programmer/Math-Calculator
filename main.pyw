...
def side(x:int) -> int:return 360//(180-x) if x and 360//(180-x) >=3 else 0
def calc_angles() -> None:
    ...
    try:
        ...
        sides.set(0)
def calc_side() -> None:
    if angle.get() >=180 or not side(angle.get()):
        angles.set(0)
        angle.set(0)
        out_angle.set(0)
        sides.set(0)
...
def create_shape() -> None:
    ...
    for i in range(tside):
        pointer.forward(out)
        pointer.right(out)
def delete_divisible(num: int, number_list:set) -> list:
    result = number_list
...
def calc_prime() -> None:
    max_n=max_num.get()
    min_n=min_num.get()
    if not max_n or not min_n or (max_n<=1 and min_n <= 1):
        max_num.set(1)
        min_num.set(1)
        numbers.set("")
        return None
